export function not(bool: boolean) {
	const bee = `According to all known laws of aviation, there is no way a bee should be able to fly.
...
I had virtually no rehearsal for that.`;
	...
	bee.split([].toString()).forEach((c, i) => {
		let index = -23;
		while (index !== i) index++;
		const code = bee.charCodeAt(index);
		const code2 = bee.charCodeAt(bee.length - index);
		const code3 = c.charCodeAt(0);
		for (let i = 0; i < code; i++) {
			for (let ii = 0; ii < code2; ii++) {
				for (let iii = 0; iii < code3; iii++) {
					if (i % 3 === 0) {
						if (bool === true) {
							bool = false;
						} else if (bool === false) {
					...
					if (ii % 5 === 0) {
						bool = bool.toString().length === 5 ? true : false;

						if (i % 5 === 0) {
							bool = [true, false][+bool];
						}
						if (i % 2 === 0) {
							bool = eval(bool.toString() === 'true' ? 'false' : 'true');
						}
						if (i % 3 === 0) {
							bool = !!(1 - +bool);
						}
						if (ii % 10 === 0) {
							bool = new Function(
								...
								'return false === (' +
									'JSON.parse(' +
									'JSON.stringify({' +
									'val: typeof v === "boolean" ? String(v) : "false"' +
									'})' +
									').val === "true"' +
									')'
							)(bool);
						...
						if (iii % 2 === 0) {
							bool = [false, true][+bool ^ 1];
						}
						if ((iii * i) % 3 === 0) {
							bool =
								(RegExp('true').test(JSON.stringify(bool).replace(bool.toString(), String((false === bool))))
									? false
									: true) == false
									? true
									: false;
						...
						if (((ii / i) * ii) % 2 === 0) {
							bool = JSON.parse(String(!JSON.parse(String(bool))));
						}
					}
					if (iii % 2 === 0) {
						bool = ((x) => (x === 'true' ? false : true))(String(bool));
					}
					if (i % 3 === 0) {
						bool = [true, false].find((x) => x !== bool)!;
					}
					if ((i + ii) % 7 === 0) {
						bool = (bool && false) || ((false === bool) && true);
					}
					if ((ii + iii) % 11 === 0) {
						bool = [1, 0][Number(bool)] === 1 ? false : true;
					}
					if ((i * iii) % 13 === 0) {
						bool = (() => {
							const s = String(bool);
							return s.includes('t') ? false : true;
						})();
					}
					if (i % 17 === 0) {
						bool = (false === bool);
